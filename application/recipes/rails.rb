node.run_state[:rails_apps].each do |app|
  ...
  env_vars.merge("RBENV_VERSION" => app["ruby_version"])
  ...
  if node[:role_names].has_value?("web")
  ...
  deploy_revision app['id'] do
    ...
    restart_command "#{app['deploy_to']}/shared/scripts/restart_app" if node[:role_names].has_value?("web")
    ...
    before_migrate do
      ...
      link "#{release_path}/config/unicorn.rb" do
        ...
        only_if { node[:role_names].has_value?("web") }
      end
    ...
    before_restart do
      bash "Precompile assets" do
        ...
        only_if nil, :environment => env_vars, :cwd => release_path do
          node[:role_names].has_value?("web") && File.exists?("./bin/rake") && `#{release_path}/bin/rake -T`.include?("rake assets:precompile")
        end
      end
    end
    ...
    after_restart do
      ...
      bash "Update crontab" do
        ...
        only_if { node[:role_names].has_value?("cron") && File.exists?("./bin/whenever") }
      end
    end
